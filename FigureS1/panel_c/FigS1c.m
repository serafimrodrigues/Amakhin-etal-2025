...
%  m-file to reproduce Fig.S1(c) of the supplemental material
%--------------------------------------------------------------------

%%% First unzip the data file 
%%% Then load CC for cell #3
% c3=load('cell3.txt');

%%% Plot CC after smoothing
wsize=(1500);
for k=1:length(wsize)
smoothc3=smoothdata(c3(1:1200000,3),'movmean',wsize(k));
end
plot(smoothc3,c3(1:1200000,2),'color',[1 0.6 0.4]);hold on;
...
ind_depol=arrayfun(@(i)find(vr{i}(Is{i,2}<I_depol(i),ip.V),1,'last'),1:nruns); 
...
ind_hopflow=arrayfun(@(i)find(vr{i}(:,ip.V)<ccruns(i).data(icv_hopflow(i),ip.V),1,'last'),1:nruns);
...
ind_fold=[ind_fold1;ind_fold2];
ind_fold=cellfun(@(x)cat(1,x,Inf(1-numel(x),1)),ind_fold);
