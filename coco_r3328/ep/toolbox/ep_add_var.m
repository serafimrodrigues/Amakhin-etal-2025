function [prob, data] = ep_add_var(prob, data, sol)
...
% SOL  : 'ep.var' instance solution structure (see also EP_READ_SOLUTION).
%        The fields SOL.var.u0 and and SOL.var.t0 must contain the initial
%        solution guess and an initial continuation direction,
%        respectively. Typically, SOL.t0=[], which means that the
%        continuation direction is determined by the covering method. A
...
% $Id: ep_add_var.m 3317 2025-01-07 21:10:49Z hdankowicz $

[prob, data] = ep_var_construct_eqn(prob, data, sol);
end
