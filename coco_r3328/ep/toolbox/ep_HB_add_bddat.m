function prob = ep_HB_add_bddat(prob, oid, labels, fhan, varargin)
no_utils = 0;
...
switch command
  ...
  case 'data'
    ...
    ep_hb = fdata.ep_hb;
    v = chart.x(uidx(ep_hb.v_idx));
    k = chart.x(uidx(ep_hb.k_idx));
    data.data.ep_hb = ep_hb;
    res = data.fhan(data.data, x, p, v, k);
end
