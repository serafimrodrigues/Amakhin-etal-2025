classdef coco_func_data < handle
    ...
    function out = pointers(cmd, in)
      ...
      switch cmd
        ...
        case 'restore'
          ...
          for i=1:numel(idxs)
            p      = ptrs(i);
            ptr(idxs(i)) = coco_func_data_ptr(p.pr,p.sh);
          end
        case 'copy'
          if nargin==2
            idxs = in;
  ...
  methods
    ...
    function obj = coco_func_data(prot, shrd, S)
      switch nargin
        ...
        case 1
          if isa(prot, 'coco_func_data')
            ...
            if ischar(prot) && ~isempty(prot)
              ...
              assert(~isempty(sidx), ...
                '%s: argument must be either ''share'' or ''protect''', mfilename);
              shmode = (sidx == 1);
              prot = [];
            end
            if isempty(prot)
            ...
            assert(isstruct(prot), '%s: argument must be a struct', mfilename);
            obj.idx = obj.pointers('new');
            ptr     = obj.pointers('get', obj.idx);
            ptr.pr  = prot;
            obj.share = shmode;
        ...
        case 2
          if isa(prot, 'coco_func_data')
            assert(ischar(shrd), ...
              '%s: second argument must be either ''share'' or ''protect''', mfilename);
            ...
            assert(isstruct(shrd), '%s: second argument must be a struct', mfilename);
    ...
    function idx = getobjidx(p)
      idx = p.idx;
    end
    
    function delete(p)
      p.pointers('delete', p.idx);
    end
    ...
    function p = protect(p, varargin)
      if nargin==1
        ...
        assert(iscellstr(varargin) || isstring(varargin), ...
