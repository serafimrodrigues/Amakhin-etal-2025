...
% TYPE_SPEC = { ('regular'|'discrete'|'singular') PAR_NAMES }
...
%
%   'regular'|'discrete : add the function to the set of monitor functions
%      that are not embedded in the continuation problem and treat events
%      associated to the corresponding continuation parameters as regular
%      special points (IFT is satisfied). Regular continuation parameters
%      do not increase the dimensional deficit of a continuation problem.
%
...
%     by regular, discrete, or singular monitor functions. 
%
...
%     function dependency index set. Can only be passed to regular,
%     discrete or singular monitor functions.
%
...
% $Id: coco_add_func.m 3324 2025-01-17 21:40:20Z hdankowicz $
...
switch type
  ...
  case { 'inactive' 'active' 'internal' 'inequality' }
    assert(efunc.close_level<1, ...
      ...
      mfilename, fid);
  case { 'regular' 'discrete' 'singular' }
...
while argidx<=nargin-2
  ...
  switch oname
    ...
    case 'passtangent'
      assert(any(strcmp(type, {'regular' 'discrete' 'singular'})), ...
        '%s: error when adding function ''%s'',\n%s', mfilename, fid, ...
        'tangent can only be passed to regular, discrete, or singular monitor functions');
      efopts.tanMode = 4;
    ...
    case { 'returnsprob' 'returnsopts' }
      assert(any(strcmp(type, {'regular' 'discrete' 'singular'})), ...
        ...
        'regular, discrete, or singular monitor functions');
      efopts.optsMode = 8;
      ...
      switch type
        ...
        case 'zero'
          ...
          assert(flag, '%s: %s %s', mfilename, ...
            ...
            'embedded monitor functions');
        case {'regular' 'discrete'}
          flag = all(ismember(idx, [efunc.embedded, efunc.regular, ...
            efunc.discrete]));
          assert(flag, '%s: %s %s', mfilename, ...
            'regular/discrete monitor functions can only require other', ...
            'regular/discrete or embedded monitor functions');
        case 'singular'
          flag = all(ismember(idx, [efunc.embedded, efunc.regular, ...
            efunc.discrete, efunc.singular]));
          assert(flag, '%s: %s %s', mfilename, ...
            'singular monitor functions can only require other', ...
            'singular, regular, discrete, or embedded monitor functions');
      end
      argidx = argidx + 1;
...
switch type
  case {'zero' 'regular' 'discrete' 'singular'}
