function prob = coco_add_event(prob, varargin)
...
% 2-4. The same type identifier can be used for several events.
%
...
% $Id: coco_add_event.m 3327 2025-01-20 02:37:28Z hdankowicz $
...
idx = find( strcmp(event.par, prob.efunc.idx2par), 1 );
if ~isempty(find(prob.efunc.('discrete_pars')==idx,1))
  if any(~isnan(event.vals))
    error('%s: event values not allowed for discrete test functions', ...
      mfilename);
  end
  event.vals = 0;
elseif any(isnan(event.vals))
  error('%s: missing event values', mfilename);
end
