...
%
%   'regular'|'discrete' : add the function to the set of complementary
%      monitor functions that are not embedded in the continuation problem
%      and treat events associated to the corresponding continuation
%      parameters as regular special points (IFT is satisfied). Regular
      ...
      switch type
        ...
        case {'regular' 'discrete'}
          flag = all(ismember(idx, [efunc.embedded, efunc.regular, ...
            efunc.discrete, complementary.embedded, ...
            complementary.discrete, complementary.regular]));
          assert(flag, '%s: %s %s', mfilename, ...
            'regular/discrete monitor functions can only require other', ...
            'regular/discrete or embedded monitor functions');
        case 'singular'
          flag = all(ismember(idx, [efunc.embedded, efunc.regular, ...
            efunc.discrete, efunc.singular, complementary.embedded, ...
            complementary.regular, complementary.discrete, 
