...
% $Id: coco_close_efunc.m 3310 2023-10-27 13:41:25Z hdankowicz $
...
if cpnum<ppnum
  ...
  errmsg = sprintf('%snumber of inactive parameters given: %d\n', errmsg, cpnum);
  errmsg = sprintf('%sAttempting to perform continuation of overdetermined problem.', errmsg);
...
efunc.op_idx     = [ cont_par_idx(1:ppnum) tmp.internal_pars(max(ipend+1,1):end) ];
tmp.cpbegin      = ppnum+1;
