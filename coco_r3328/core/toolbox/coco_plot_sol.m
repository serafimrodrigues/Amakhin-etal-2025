...
%
%   COCO_PLOT_SOL([AH], [THEME], RUN, [LABS], OID, [OIDX], [COL1, [IDX1], [COL2, [IDX2], [COL3, [IDX3]]]])
%
...
%
%   COCO_PLOT_SOL(AH, ...) uses the provided axes handle for plotting.
%
...
grammar   = '[AH] [THM] RUN [LABS] OID [OIDX] [COL1 [IDX1] [COL2 [IDX2] [COL3 [IDX3]]]]';
args_spec = {
    'AH',     '',   'axes',    'ah',   gca, 'read', {}
...
if nargin(thm.plot_sol)==6
  thm = thm.plot_sol(args.ah, thm, args, dim, bd, HH);
else
  thm = thm.plot_sol(thm, args, dim, bd, HH);
end

if thm.axlabs
  xlabel(args.ah, thm.xlab, thm.xlstyle{:});
  ylabel(args.ah, thm.ylab, thm.ylstyle{:});
  zlabel(args.ah, thm.zlab, thm.zlstyle{:});
end
