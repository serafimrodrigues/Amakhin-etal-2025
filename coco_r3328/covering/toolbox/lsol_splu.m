...
if mA~=nA
  ...
  [mB, nB] = size(chart.NV);
    ...
    error('missing NV field in chart')
  end
  if mA>nA
    A = [A [chart.NV; zeros(mA-mB,nB)]];
  elseif nA>mA
    A = [A; [chart.NV; zeros(nA-mB,nB)]'];
  end
end
...
if prob.lsol.det
  sdata = coco_get_slot_data(prob, 'set_mode', 'lsol.det', 'data');
  if sdata.save_det
    if issquare
      DR  = full(diag(R));
      SR  = prod(sign(DR));
