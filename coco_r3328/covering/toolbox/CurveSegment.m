...
%       TS = orth(full(TS));
  ...
  methods (Access=public) % interpolation functions
    ...
    function [opts, cseg, TS] = init_TS(cseg, opts, Valpha, h0, N, branch)
      ...
      if strcmp(branch, 'reset')
        t0 = zeros(size(t0));
      elseif any(strcmp(branch, {'switch', 'no switch'}))
        if ~coco_exist('NullItMX', 'class_prop', opts, 'cont')
          ...
          opts = coco_set(opts, 'cont', 'NullItMX', 1);
        end
      end
      ...
      if all(t0 == 0)
        ...
        if strcmp(branch, 'switch') %find candidate direction for secondary branch (dim = 1 only?)
          fidx = coco_get_func_data(opts, 'cseg.prcond', 'fidx');
