...
%%
%#ok<*AGROW>
default={'funcname','sys','keeptemp',false,...
    'outname','out','multifile',false,'rename',true,'inname','in'};
...
vars=[x(:).',dx(:).'];
v_orig_names=sco_names_from_sym(vars);
nf=length(df{1});
%% rename inputargs if requested (default to avoid clashes)
if options.rename
    newvar=sym(options.inname,[1,length(vars)]);
    for i=1:length(df)
        df{i}=subs(df{i},vars,newvar);
    end
    vars=newvar;
end
...
if ~isempty(intersect(vnames,v_orig_names))
    error('dde_symcode:names',...
        'dde_symcode: name clash between input names and variables');
end
if sco_isoctave()
