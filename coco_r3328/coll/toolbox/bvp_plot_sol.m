function thm = bvp_plot_sol(ah, thm, args, dim, bd, HH)
...
for i=1:numel(idx)
  ...
  for j=1:numel(args.oid)
    sol = bvp_read_solution(args.oid{j}, args.run, lab);
    o = ones(size(sol{1}.tbp'));
    for l=1:numel(sol)
      sol{l}.tau = sol{l}.tbp/sol{l}.T;
      for k=1:numel(x)
        x{k} = x_f{k}(sol{l},row,o);
      end
      for k=1:numel(y)
        y{k} = y_f{k}(sol{l},row,o);
      end
      if dim==2
        plot(ah, args.idx1(x{:}), args.idx2(y{:}), lspec{:});
      else
        for k=1:numel(z)
          z{k} = z_f{k}(sol{l},row,o);
        end
