...
switch format
  ...
  case 'coll.v1'
    ...
    [data, chart1] = coco_read_adjoint(tbid, run, lab, 'data', 'chart');
    ...
    if ~isempty(data.pnames)
      chart2 = coco_read_adjoint(coco_get_id(tbid, 'pars'), run, ...
        lab, 'chart');
    ...
    if data.ode.autonomous
      chart3 = coco_read_adjoint(coco_get_id(tbid, 'T0'), run, ...
        lab, 'chart');
      sol.T0_l0 = chart3.x;
      sol.T0_tl = chart3.t;
