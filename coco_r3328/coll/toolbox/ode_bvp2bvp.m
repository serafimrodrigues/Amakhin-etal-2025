...
% OPTS = { '-bvp-end' | '-end-bvp' | '-var' VECS | '-no-var' | '-no-pars'}
%
...
%
% OPTS : '-bvp-end', '-end-bvp', '-var' VECS, and '-no-var' (optional,
%        multiple options may be given). Either '-bvp-end' or '-end-bvp'
%        marks the end of input to ODE_BVP2BVP. The option '-var' indicates
%        the inclusion of a variational problem for each trajectory
%        segment, where the initial solution guess for the perturbations to
%        the initial conditions of the i-th trajectory segment is given by
%        the content of the i-th element of the cell array VECS.
%        Alternatively, the option '-no-var' indicates the exclusion of the
%        variational problem even if this were included in the continuation
%        run RUN.
%
...
for i=1:nsegs
  ...
  if ~opts.novar
    ...
    prob = ode_coll2coll(prob, toid, args.run, soid, args.lab, '-no-var');
  end
end
data.cids = cids;
