function [prob, data] = coll_add_var(prob, data, sol)
%COLL_ADD_VAR   Append 'coll' instance variational problem.
%
% [PROB DATA] = COLL_ADD_VAR(PROB, DATA, SOL [OPTS])
% OPTS = {}
%
...
% SOL  : 'seg.var' instance solution structure (see also 
%        COLL_READ_SOLUTION). The fields SOL.var.u0 and and SOL.var.t0 must
%        contain the initial solution guess and an initial continuation
%        direction, respectively. Typically, SOL.t0=[], which means that
%        the continuation direction is determined by the covering method. A
%        non-empty vector SOL.t0 must be given for branch-switching at
%        branch points.
%
...
% $Id: coll_add_var.m 3317 2025-01-07 21:10:49Z hdankowicz $

[prob, data] = coll_var_construct_eqn(prob, data, sol);
end
