...
coll_args = {@linode, @linode_dx, @linode_dp, @linode_dxdx, ...
  @linode_dxdp, @linode_dpdp, t0, x0, [0.98; 0.3]};
bvp_args  = [coll_args, {{'k' 'th'}, @linode_bc, @linode_bc_du, ...
  @linode_bc_dudu}];
prob1 = ode_isol2bvp(prob1, '', bvp_args{:});

[data, uidx] = coco_get_func_data(prob1, 'bvp.seg1.coll', 'data', 'uidx');
...
[data, axidx] = coco_get_adjt_data(prob1, 'bvp.seg1.coll', 'data', 'axidx');
opt = data.coll_opt;
...
% zero problems
prob2 = ode_bvp2bvp(prob, '', 'linode1', BPlab(1));

[data, uidx] = coco_get_func_data(prob2, 'bvp.seg1.coll', 'data', 'uidx');
maps  = data.coll_seg.maps;
prob2 = coco_add_pars(prob2, 'vel', uidx(maps.x0_idx(2)), 'v');

% adjoints
prob2 = adjt_bvp2bvp(prob2, '', 'linode1', BPlab(1));

[data, axidx] = coco_get_adjt_data(prob2, 'bvp.seg1.coll', 'data', 'axidx');
opt   = data.coll_opt;
...
% zero problem
prob3 = ode_bvp2bvp(prob, '', 'linode2', lab(2));

[data, uidx] = coco_get_func_data(prob3, 'bvp.seg1.coll', 'data', 'uidx');
maps = data.coll_seg.maps;
prob3 = coco_add_pars(prob3, 'vel', uidx(maps.x0_idx(2)), 'v');

% adjoint
prob3 = adjt_bvp2bvp(prob3, '', 'linode2', lab(2));

[data, axidx] = coco_get_adjt_data(prob3, 'bvp.seg1.coll', 'data', 'axidx');
opt = data.coll_opt;
