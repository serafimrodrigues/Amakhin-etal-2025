...
% OPTS = { '-hspo-end' | '-end-hspo' | '-var' VECS | '-no-var'}
%
...
%
% OPTS : '-hspo-end' '-end-hspo', '-var' VECS, and '-no-var' (optional,
%        multiple options may be given). Either '-hspo-end' or '-end-hspo'
%        mark the end of input to ODE_HSPO2HSPO. The option '-var'
%        indicates the inclusion of a variational problem for each
%        trajectory segment, where the initial solution guess for the
%        perturbations to the initial conditions of the i-th trajectory
%        segment is given by the content of the i-th element of the cell
%        array VECS. Alternatively, the option '-no-var' indicates
...
if ~opts.novar
  ...
  prob = ode_bvp2bvp(prob, tsid, args.run, ssid, args.lab, '-no-var');
end
